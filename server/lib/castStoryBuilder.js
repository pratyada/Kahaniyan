...
function describe(c) {
  ...
  if (r === 'imaginary') return `${c.name}, who only ${'{heroName}'} could see`;
  if (r === 'friend') return `${c.name}, ${'{heroName}'}'s very best friend`;
...
function beatFor(c, contextNoun = 'the path') {
  ...
  if (r === 'pet') {
    ...
    return `${name} ${pet.walk} {heroName}, and every few steps said "${pet.sound}" — which, in pet language, meant "I am here, and I am yours." Then ${name} ${pet.move}, and everyone smiled, because ${name} always knew exactly when the moment needed a little softness.`;
  }
  if (r === 'imaginary') {
    return `${name} skipped along beside {heroName}, sometimes visible, sometimes not, but always close. "I will go first," ${name} whispered. "I am made of bravery and starlight, and that is enough for both of us."`;
  }
  ...
  if (r === 'dada' || r === 'nana' || r === 'grandfather') {
    return `${name} walked slowly, the way old wise people walk, his hand resting lightly on {heroName}'s shoulder. "I have walked this same path a thousand times," ${name} said gently, "and every time, the world has had something new to say."`;
  }
  if (r === 'dadi' || r === 'nani' || r === 'grandmother') {
    return `${name} stopped to point at a small wildflower beside ${contextNoun}. "Do you know its name?" she asked softly. {heroName} did not. "It is called ${ ['the moon-tear flower', 'the night-singer', 'the small candle'][Math.floor(Math.random()*3)] }," ${name} smiled. "It only blooms for children who are paying attention."`;
  }
  if (r === 'daddy' || r === 'mummy') {
  ...
  if (r === 'friend') {
    return `${name}, who was {heroName}'s very best friend, walked along close enough that their shoulders bumped sometimes. They did not need to talk much. The best friendships are the ones where the silence is also a kind of speaking.`;
  }
...
function scaffoldGardenAdventure({ childName, cast, value }) {
  ...
  return `It was the kind of evening when the sky goes the colour of warm honey, and the garden behind ${heroName}'s house grew quiet with bees.

That night, ${heroName} was not alone. With them came ${intro}.

Together, they walked into the garden to look for something — though ${heroName} could not have said exactly what. Sometimes the most important things have no name until you find them.

${beats.join('\n\n')}

At the very end of the garden, by an old stone bench that nobody used anymore, ${heroName} found it — the small thing they had been looking for all along. It was not a treasure. It was not a key. It was ${phrase}, and it was waiting just for them.
...
That night, ${heroName}, when you close your eyes, remember the garden, and the people who walked it with you, and the small thing you found together. Sleep well, little one. The garden is still there. The honey-coloured sky is waiting for tomorrow.`;
}
...
function scaffoldLostTreasure({ childName, cast, value }) {
  ...
  return `One bright morning, ${heroName} realized something very important was missing. It was not a toy. It was not a coin. It was something quieter — a feeling, perhaps, or a small certainty about how the world worked.

${heroName} did not know where to look. So they did the only sensible thing: they asked their family.
...
Hours passed. The sun grew low. ${heroName}'s feet were tired, and so were everyone else's. They sat down on a smooth flat stone and looked at each other.

And then, in the middle of all that quiet looking, ${heroName} understood. The missing thing had not been lost out in the world. It had been here all along — in the people who had walked beside them, in their slowness, in their willingness to look together for something that did not even have a name. The missing thing was ${phrase}, and they had found it without even noticing.

${VALUE_LESSONS[value] || VALUE_LESSONS.kindness}

That night, ${heroName}, when you close your eyes, remember that the most important things do not stay missing for long when you have people who will walk beside you. Goodnight, little adventurer. Tomorrow has more stones to sit on, and more roads to walk slowly down.`;
}
...
It was a festival night. And ${heroName}, with ${intro}, was right in the middle of it all.

${beats.join('\n\n')}

When all the lamps had been lit and the courtyard was glowing like a small piece of the sky had fallen down to earth, ${heroName} stood very still and looked at everything.

So many lights. So many faces. So many small kindnesses, each one barely noticed, each one important.

${heroName} understood, then, that the festival was not really about the lamps. The festival was about ${phrase}. The lamps were just a way of pointing at it, so that nobody could miss it.

${VALUE_LESSONS[value] || VALUE_LESSONS.kindness}

That night, ${heroName}, as you fall asleep, imagine the courtyard glowing like a little galaxy, with everyone you love standing inside it. That is what a real festival is. Sweet dreams, little light. The lamps will burn until morning.`;
}
...
function scaffoldRooftopStargazers({ childName, cast, value }) {
  ...
  return `On certain nights — the very best nights — ${heroName} climbed up to the flat rooftop of the house and looked at the stars. Tonight was one of those nights. And tonight, ${heroName} was not alone. With them, climbing up the narrow stairs one by one, came ${intro}.
...
${heroName} lay back on a folded blanket and looked straight up. The stars looked back. They had been doing this — looking back at children — for a very, very long time.

After a while, ${heroName} understood something important. The whole sky was made of ${phrase}. Every star was a small one. Every star, on its own, looked like nothing. But all of them together made the night beautiful enough to remember forever.

${VALUE_LESSONS[value] || VALUE_LESSONS.kindness}

That night, ${heroName}, when you finally close your eyes, the stars will still be up there. They are not going anywhere. And neither are the people who climbed up to the rooftop with you. Goodnight, little stargazer. The sky has been waiting for you, and it will wait again tomorrow.`;
}
...
export function buildCastStory({ childName, gender, cast, value, recentPlotTypes = [] }) {
  ...
  // Replace {heroName} tokens and fix pronouns by gender
  let cleaned = text.replace(/\{heroName\}/g, childName || 'little one');
  if (gender === 'boy') {
    cleaned = cleaned.replace(/\btheir\b/g, 'his').replace(/\bthey\b/g, 'he').replace(/\bthem\b(?= tight| close| gently| softly)/g, 'him');
  } else if (gender === 'girl') {
    cleaned = cleaned.replace(/\btheir\b/g, 'her').replace(/\bthey\b/g, 'she').replace(/\bthem\b(?= tight| close| gently| softly)/g, 'her');
  }
