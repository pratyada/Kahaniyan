    ...
    <PageTransition className="flex flex-col h-screen bg-bg-base safe-top">
      ...
      {(promptImage || prompt) && (
        <div className="px-5 pb-3">
          {promptImage && (
            <div className="rounded-2xl overflow-hidden ring-1 ring-white/10 mb-2" style={{ maxHeight: 200 }}>
              <img src={promptImage} alt={prompt} className="w-full h-full object-cover" />
            </div>
          )}
          {prompt && !promptImage && (
            <div className="rounded-2xl bg-bg-surface p-4 ring-1 ring-white/5 text-center">
              <p className="text-lg font-bold text-gold" style={{ fontFamily: 'Lora, serif' }}>"{prompt}"</p>
            </div>
          )}
        </div>
      )}
