...
export default function StonedAge() {
  ...
  // Show login popup after 5 seconds if not signed in
  useEffect(() => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#f5f0e8', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
      ...
      {/* Login popup — appears after 5 seconds */}
      <AnimatePresence>
        {showLogin && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          >
            <motion.div
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              ...
              style={{ width: '100%', maxWidth: 460, background: '#1a1a28', borderRadius: '24px 24px 0 0', padding: 28, textAlign: 'center' }}
            >
              <div style={{ width: 40, height: 4, borderRadius: 999, background: 'rgba(255,255,255,0.15)', margin: '0 auto 20px' }} />
              <div style={{ fontSize: 40, marginBottom: 8 }}>🪨</div>
              <h2 style={{ fontFamily: 'Playfair Display,Georgia,serif', fontSize: 22, fontWeight: 700, color: '#f0a500' }}>Sign in to continue</h2>
              <p style={{ fontSize: 13, color: '#a8a39a', marginTop: 8 }}>The vibes are free. We just need to know who's on the couch.</p>
              <button
                onClick={async () => { if (auth && googleProvider) try { await signInWithPopup(auth, googleProvider); } catch {} }}
                style={{ marginTop: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, width: '100%', padding: '14px 24px', borderRadius: 999, background: '#fff', color: '#333', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer' }}
              >
