...
export default function StonedAge() {
  ...
  if (isConfigured && !user) {
    return (
      <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#f5f0e8', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <motion.div style={{ fontSize: 64, marginBottom: 16 }} animate={{ rotate: [0, 10, -10, 5, -5, 0], scale: [1, 1.1, 0.95, 1.05, 1] }} transition={{ duration: 6, repeat: Infinity }}>🪨</motion.div>
        <h1 style={{ fontFamily: 'Playfair Display,Georgia,serif', fontSize: 32, fontWeight: 700 }}><span style={{ color: '#f0a500' }}>Stoned</span> Age</h1>
        <p style={{ color: '#a8a39a', fontSize: 14, marginTop: 8, textAlign: 'center', maxWidth: 300 }}>After kids sleep, parents play. Sign in to unlock the vibes.</p>
        <button
          onClick={async () => { if (auth && googleProvider) try { await signInWithPopup(auth, googleProvider); } catch {} }}
          style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 10, padding: '14px 28px', borderRadius: 999, background: '#fff', color: '#333', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer' }}
        >
        ...
        <a href="https://mysleepytale.com" style={{ marginTop: 20, fontSize: 12, color: '#6e6a63', textDecoration: 'none' }}>← Back to {APP_NAME}</a>
      </div>
    );
  }
