...
export default function Login() {
  ...
  const { loginEmail, signupEmail, loginGoogle, resetPassword, error, clearError } = useAuth();
    ...
    <div className="phone-shell">
      ...
      <div className="relative z-10 flex h-full flex-col justify-center px-6 safe-top safe-bottom">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <span className="text-[10px] uppercase tracking-wider text-ink-dim">{t('login.or')}</span>
          <div className="h-px flex-1 bg-white/10" />
        </div>
        ...
        >
          ...
          {verifyMsg && (
            <div className="rounded-xl bg-green-500/10 px-3 py-2 text-[12px] text-green-400">
              {verifyMsg}
            </div>
          )}
          ...
          {mode === 'login' && (
            <button type="button" onClick={() => { setMode('reset'); clearError(); setResetSent(false); }}
              className="w-full text-center text-[11px] text-ink-muted">
              {t('login.forgotPassword')}
            </button>
          )}
          {mode === 'reset' && (
            <button type="button" onClick={() => { setMode('login'); clearError(); setResetSent(false); }}
              className="w-full text-center text-[11px] text-ink-muted">
              Back to login
            </button>
          )}
