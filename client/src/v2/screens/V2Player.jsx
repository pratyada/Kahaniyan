// V2 Player — night-sky playback. Plays current.audioUrl DIRECTLY via loadCached
// (instant, no hash check). Multilingual stories get a language picker (translate
// + TTS). Records the play so the "already played" outline shows on cards.
...
export default function V2Player() {
  ...
  // Reset language when the story changes
  useEffect(() => { setLang('English'); }, [current?.id]);

  // Start playback when story OR language changes
  useEffect(() => {
    if (!current) return;
    ...
    (async () => {
      ...
      if (lang === 'English' && current.audioUrl) {
        ...
        if (lang !== 'English') {
          ...
          } catch { /* fall back to English text */ }
        }
  ...
  return (
    <div className="px-5 lg:px-8 pt-6 pb-28 max-w-[560px] mx-auto">
      ...
      {/* Language picker (multilingual only) */}
      {multi && (
        <div className="mt-5">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
            {langs.map(([name, flag]) => (
              <button
                ...
                className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-bold transition ${lang === name ? 'text-[#0D1B2A]' : 'text-[#B8AAC8] bg-white/[0.06] ring-1 ring-white/10'}`}
                style={lang === name ? { background: GOLD } : undefined}
              >
                {flag} {name}
              </button>
            ))}
