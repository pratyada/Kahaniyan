...
// Shows "Coming Soon" overlay for unreleased series.

import { motion } from 'framer-motion';
import { Play, Lock } from 'lucide-react';
import { useSeriesProgress } from '../../hooks/useSeriesProgress.js';

export default function SeriesCard({ series, coverImage, onClick }) {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={comingSoon}
      className={`group relative shrink-0 snap-start ${comingSoon ? 'cursor-default' : ''}`}
    ...
    >
      ...
      >
        ...
        {coverImage ? (
          <img src={coverImage} alt="" className="absolute inset-0 h-full w-full object-cover" style={comingSoon ? { filter: 'grayscale(0.6) brightness(0.5)' } : {}} onError={(e) => { e.target.style.display = 'none'; }} />
        ) : (
          <div className="absolute inset-0" style={{ background: series.gradient, ...(comingSoon ? { filter: 'grayscale(0.4) brightness(0.5)' } : {}) }} />
        )}
        ...
        {comingSoon && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/40">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-white/10 backdrop-blur-sm ring-1 ring-white/20 mb-2">
              <Lock size={16} className="text-white/60" />
            </div>
            <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-white/70">Coming Soon</span>
          </div>
        )}

        {/* Series icon (large, centered top) — hidden when coming soon */}
        {!comingSoon && (
        ...
        {/* Play badge — only for live series */}
        {!comingSoon && (
        ...
        <div className={`absolute bottom-0 left-0 right-0 p-3 ${comingSoon ? 'z-20' : ''}`}>
          ...
          {series.creatorName && (
            <p className="text-[9px] text-gold/60 mb-1">by {series.creatorName}</p>
          )}

          {/* Progress bar — only for live series */}
          {!comingSoon && (
