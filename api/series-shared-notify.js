// Series shared notification — sent when owner shares a personal series
// POST /api/series-shared-notify { seriesTitle, seriesId, ownerName, sharedWithEmail }
...
export default async function handler(req, res) {
  ...
  const { seriesTitle, seriesId, ownerName, sharedWithEmail } = req.body || {};

  if (!seriesTitle || !seriesId || !sharedWithEmail) {
    return res.status(400).json({ error: 'Missing seriesTitle, seriesId, or sharedWithEmail' });
  }

  const subject = `${ownerName || 'Someone'} shared a bedtime story series with you`;
  const html = wrap('A Story Series Awaits You!', `
    <p style="font-size:15px;line-height:1.7;margin:0 0 12px;">
      <strong style="color:#f5f0e8;">${ownerName || 'A friend'}</strong> has shared a bedtime story series with you!
    </p>
    <div style="background:#f0a50010;border:1px solid #f0a50030;border-radius:12px;padding:16px;margin-bottom:16px;">
      ...
      <p style="font-size:16px;font-weight:bold;color:#f0a500;margin:0;">${seriesTitle}</p>
    </div>
    <p style="font-size:14px;color:#a8a39a;margin:0 0 16px;">
      Sign up with this email to access the series and listen to every episode at bedtime.
    </p>
  ...
  `);
  const text = `${ownerName || 'Someone'} shared the bedtime story series "${seriesTitle}" with you! Sign up with this email to access it. Listen: https://mysleepytale.com/series/${seriesId}`;

  try {
    await sendEmail(sharedWithEmail, subject, html, text);
