// Shared OG/social-preview data + rendering helpers.
// Used by api/share.js (per-story share links: /api/share?id=<id>) and
// api/og.js (dynamic rendering for social crawlers, driven by the CloudFront
// viewer-request function infra/cloudfront-og-router.js).
//
// Keeping story resolution + HTML escaping in one place means share.js and og.js
// never drift apart. All dynamic text is HTML-escaped before it reaches a
// meta tag (no injection via story titles/descriptions).

// Auto-generated from client/src/data/culturalLessons.js — covers ALL wisdom
...
// Default fallback image — our own generated image, never Unsplash.
export const DEFAULT_OG_IMAGE = 'https://mysleepytale.com/dsp_ep1_development.png';
export const SITE = 'https://mysleepytale.com';

let imageCache = null;
let galleryCache = null;
let cacheTime = 0;

// Escape text destined for an HTML attribute / element so story data can never
// break out of a meta tag or inject markup.
...
export async function getWisdomImages() {
  const now = Date.now();
  ...
  } catch (e) { console.error('[og-data] Firestore error:', e.message); return { images: imageCache || {}, gallery: galleryCache || {} }; }
}

// Hand-kept OG map — wins over WISDOM_TITLES because it carries series/episode/
// ogImage extras that the generated file does not have.
export const TITLES = {
  ...
  ps_ep9_blackhole: { title: 'What Is a Black Hole?', tradition: 'Universal', duration: 5, series: 'Planets & Stars', ep: 9, totalEp: 9, ogImage: 'https://mysleepytale.com/media/stories/ps_ep9_blackhole.jpg' },
  ...
  'brave-moments': { title: 'Brave Moments That Changed the World', tradition: 'Universal', duration: null, isSeries: true, totalEp: 8, description: 'Real stories of ordinary courage — when one person or one moment made powerful people listen and changed the world forever.', firstEpId: 'brave_ep1_che', seriesUrl: '/series/brave-moments' },
  brave_ep1_che: { title: 'The Doctor Who Chose the Poor', tradition: 'Universal', duration: 5, series: 'Brave Moments That Changed the World', ep: 1, totalEp: 8 },
  brave_ep2_mandela: { title: 'Twenty-Seven Years and One Vote', tradition: 'Universal', duration: 5, series: 'Brave Moments That Changed the World', ep: 2, totalEp: 8 },
  brave_ep3_gandhi: { title: 'The Salt That Shook an Empire', tradition: 'Hindu', duration: 5, series: 'Brave Moments That Changed the World', ep: 3, totalEp: 8 },
  brave_ep4_shastri: { title: 'The Smallest Prime Minister', tradition: 'Hindu', duration: 5, series: 'Brave Moments That Changed the World', ep: 4, totalEp: 8 },
  ...
  'dr-spock-parenting': { title: "Dr. Sleepy's Parenting Corner", tradition: 'Universal', duration: null, isSeries: true, totalEp: 6, description: "Bedtime conversations with Dr. Sleepy — inspired by Dr. Spock's timeless parenting wisdom.", firstEpId: 'dsp_ep1_development', seriesUrl: '/series/dr-spock-parenting' },
  dsp_ep6_wildfires: { title: "When the Sky Turns Orange", tradition: 'Universal', duration: 3, series: "Dr. Sleepy's Parenting Corner", ep: 6, totalEp: 6 },
...
// ─────────────────────────────────────────────────────────────────────────────
// Resolve a story / series / collection id → OG meta.
// Mirrors the resolution chain that api/share.js has always used:
//   hand-kept TITLES (series/episode/ogImage extras) → generated WISDOM_TITLES →
//   Firestore creatorSeries → productionStories → publishedContent (Admin SDK) →
//   publishedContent (REST) → static story path w/ real cover from wisdomImages.
...
export async function resolveStory(storyId) {
  ...
  // Firestore creatorSeries (public REST)
  if (!story) {
    try {
      const fsRes = await fetch(`${FIRESTORE_URL}/creatorSeries/${id}`);
      if (fsRes.ok) {
        ...
        redirectUrl = `${SITE}/series/${id}`;
        found = true;
      }
    } catch {}
  }

  // productionStories (Admin SDK)
  if (!title) {
    try {
      const { getFirestore: getFs } = await import('./_firebase.js');
      const adminDb = await getFs();
      if (adminDb) {
        const prodSnap = await adminDb.collection('productionStories').doc(id).get();
        if (prodSnap.exists) {
  ...
  // publishedContent (Admin SDK)
  if (!title) {
    try {
      const { getFirestore: getFs } = await import('./_firebase.js');
      const adminDb = await getFs();
      if (adminDb) {
        const snap = await adminDb.collection('publishedContent').doc(id).get();
        if (snap.exists) {
          const pub = snap.data();
          title = pub.title;
  ...
  // publishedContent (public REST fallback)
  if (!title) {
    try {
      const fsRes = await fetch(`${FIRESTORE_URL}/publishedContent/${id}`);
      if (fsRes.ok) {
        const data = await fsRes.json();
        const f = data.fields || {};
  ...
  // Static story path (from TITLES / WISDOM_TITLES) with real cover image.
  if (!title) {
    ...
    if (story?.ogImage) {
      ...
      image = wisdomImages[imageKey] || wisdomImages[id] || '';
      if (!image) {
        const galleryPhotos = wisdomGallery[imageKey] || wisdomGallery[id] || wisdomGallery[lessonId] || [];
    ...
    redirectUrl = isSeries
      ? `${SITE}${story.seriesUrl}`
      : `${SITE}/story/${id || lessonId}`;
  }

  return { title, description, image, redirectUrl, found };
}

// Resolve a kid-created story (kidStories/{id}) — ONLY if approved/published, so
// private/draft recordings never leak into a public social preview.
// Returns meta or null. Redirects to the V2 player where kid stories play.
export async function resolveKidStory(storyId) {
  const id = (storyId || '').startsWith('lesson_') ? storyId.slice(7) : (storyId || '');
  if (!id) return null;
  try {
    const { getFirestore: getFs } = await import('./_firebase.js');
    const adminDb = await getFs();
    if (!adminDb) return null;
    const snap = await adminDb.collection('kidStories').doc(id).get();
    if (!snap.exists) return null;
    const k = snap.data();
    const isPublic = k.status === 'published' || k.approved === true;
    if (!isPublic) return null;
    const title = k.title || 'A little story';
    const by = k.kidName ? ` by ${k.kidName}` : '';
    const description = `A bedtime story${by}, created and narrated by a young creator on My Sleepy Tale. 🌙`;
    // Kid stories are image-or-video; use the prompt image as the still preview.
    const image = k.promptImageUrl || DEFAULT_OG_IMAGE;
    return {
      title,
      description,
      image,
      redirectUrl: `${SITE}/v2/player/${id}`,
      found: true,
    };
  } catch { return null; }
}

// Build a full, minimal HTML document carrying OG/Twitter/canonical meta, then a
// <script> + <noscript> redirect so a *human* who lands here goes to the SPA
// route while crawlers read the tags. All values are HTML-escaped by the caller
// path or here.
export function renderOgHtml({ title, description, image, canonical, redirectUrl, type = 'website' }) {
  const t = escapeHtml(title);
  const d = escapeHtml(description);
  const img = escapeHtml(image || DEFAULT_OG_IMAGE);
  const url = escapeHtml(canonical || SITE);
  const redir = escapeHtml(redirectUrl || canonical || SITE);
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <noscript><meta http-equiv="refresh" content="0;url=${redir}"></noscript>
</head>
<body>
